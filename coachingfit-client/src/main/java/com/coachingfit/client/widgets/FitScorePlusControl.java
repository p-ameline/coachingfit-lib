 ...
 * 
 */
public class FitScorePlusControl extends FitScoreControl
{
	// Controls
   ...
   */
  @Inject
  public FitScorePlusControl(final String sPath) {
  	super(sPath) ;
  }
   ...
   */
	...
	/**
   * Return a (button) style that corresponds to a given score (for example "fitred" (red background) for "0")
   *
   */
	...
	public void initIfUnset(final String sSeniority)
	{
		...
		// If the expected score is semi-fit, set it if auto-semi-fit (fit is not allowed)
		//
		if (sSeniority.compareTo(_sExpectedSemiFit) >= 0)
		{
