 ...
 * 
 */
public class GlobalFitScoreControl extends DoubleBox implements ControlModel
{
	protected class LocalControlBase extends ControlBaseWithParams
	{
		...
		public void fillParam(final String sParam, final String sValue)
		{
			...
			if      ("target".equalsIgnoreCase(sParam))
				_sTargetRoot = sVal ; 
			else if ("mode".equalsIgnoreCase(sParam))
			{
				if      ("exact".equalsIgnoreCase(sVal))
					_iMode = MODE.exactCount ;
				else if ("percentFromGlobal".equalsIgnoreCase(sVal))
					_iMode = MODE.percentFromGlobal ;
				else if ("percentFromExpected".equalsIgnoreCase(sVal))
					_iMode = MODE.percentFromExpected ;
				else
					_iMode = MODE.exactCount ;
			}
		}
	...
	public enum MODE { exactCount, percentFromGlobal, percentFromExpected } ;

	protected LocalControlBase _base ;

	protected String           _sTargetRoot ;
	protected MODE             _iMode ;

	/**
	 * Default Constructor
	 *
	 */
	public GlobalFitScoreControl(String sPath)
	{
		super() ;
	...
	/**
	 * Get content as FormDataData if content is a double or "ND", <code>null</code> if not
	 *
	 */
	public FormDataData getContent()
	{
		...
		if ((MODE.percentFromGlobal == _iMode) || (MODE.percentFromExpected == _iMode))
			formData.setUnit("%") ;
