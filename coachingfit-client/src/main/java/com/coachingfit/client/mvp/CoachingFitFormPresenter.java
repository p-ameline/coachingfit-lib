...
public class CoachingFitFormPresenter extends FormPresenterModel<CoachingFitFormPresenter.Display> implements FormPresenterInterface
{
    ...
    {
        void                             setTrainees(List<TraineeData> trainees, boolean bAllowedMode) ;
    ...
    protected class FixTraineeListCallback implements AsyncCallback<GetCoachingFitJobs4ArchetypeResult> 
    {
        ...
        public void onFailure(Throwable cause)
        {
            ...
            display.setTrainees(getTrainees(), _bAllowedMode) ;
        }
        ...
        {
            _aJobsTypes.clear() ;
            _aJobsTypes.addAll(value.getJobsTypes()) ;
     ...
     */
    private List<TraineeData> getTrainees()
    {
        List<TraineeData> aTraineesForUser = new ArrayList<TraineeData>() ;
        aTraineesForUser.addAll(((CoachingFitSupervisor) _supervisor).getTraineesForUser()) ;
                
        if (_bAllowedMode)
            for (TraineeData trainee : ((CoachingFitSupervisor) _supervisor).getAllowedTraineesForUser())
                if (false == aTraineesForUser.contains(trainee))
                    aTraineesForUser.add(trainee) ;
        
        return aTraineesForUser ;
    }
    
    /**
     * Update the list of trainees when a new list of {@link ArchetypeForJobData} is available
     */
    private void updateTraineesList()
    {
        List<TraineeData> aTraineesForUser = getTrainees() ;

        // If there is no jobs types returned for current archetype, then keep the full trainees list
        //
        if (_aJobsTypes.isEmpty() || aTraineesForUser.isEmpty())
        {
            display.setTrainees(aTraineesForUser, _bAllowedMode) ;
            return ;
        }

        List<TraineeData> aSelectedTrainees = new ArrayList<TraineeData>() ; 

        for (ArchetypeForJobData jobType : _aJobsTypes)
        {
        ...
        display.setTrainees(aSelectedTrainees, _bAllowedMode) ;
    }
     ...
     */
    private void addTrainee(List<TraineeData> aSelectedTrainees, final TraineeData trainee)
    {
